// ═══════════════════════════════════════════════════════════════
// SUNTREX — AI Support Chat (Netlify Function)
// Endpoint: POST /.netlify/functions/support-chat-ai
// ═══════════════════════════════════════════════════════════════
...
// Product catalog is also stored in src/data/suntrex-ai-catalog.md for reference.
// Inlined here because Netlify Functions bundle into a temp directory where
// relative file paths are not preserved.
...
export const handler = async (event) => {
  ...
  try {
    const { conversation_id, message, context = {} } = JSON.parse(event.body);

    if (!message || !conversation_id) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Missing message or conversation_id' }),
      };
    }
