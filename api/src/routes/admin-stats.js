/**
 * admin-stats.js — SUNTREX Admin Dashboard Data (Fastify)
 *
...
async function routes(fastify) {
  fastify.post("/admin-stats", async (request, reply) => {
    try {
      ...
      if (!isAdmin(user.email)) return reply.code(403).send({ success: false, error: "Forbidden — not an admin" });
      ...
      switch (action) {
        case "overview":
          return reply.code(200).send({ success: true, data: await fetchOverview(supabase) });

        case "transactions":
          return reply.code(200).send({ success: true, data: await fetchTransactions(supabase) });

        case "sellers":
          return reply.code(200).send({ success: true, data: await fetchSellers(supabase) });

        case "monthly":
          return reply.code(200).send({ success: true, data: await fetchMonthly(supabase) });
        ...
        case "all": {
          ...
          return reply.code(200).send({ success: true, data: { kpi: overview, transactions, sellers, monthlyRevenue: monthly } });
        }
