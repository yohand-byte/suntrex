...
async function routes(fastify) {
  fastify.post("/blog-ai-generate", async (request, reply) => {
    try {
      const { topic, category } = request.body || {};

      if (!topic || !category) {
        return reply.code(400).send({ error: "Missing topic or category" });
      }
      ...
      if (dbError) {
        ...
        return reply.code(200).send({ article, saved: false, error: dbError.message });
      }

      return reply.code(200).send({ article: saved, saved: true });
    } catch (err) {
