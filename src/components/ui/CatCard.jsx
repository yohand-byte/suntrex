...
export default function CatCard({ img, title, sub, count, big, small, onClick, montage, bg, buttonLabel, mobileHeight }) {
  ...
  if (montage) {
    return (
      <div className="hl" onClick={onClick} style={{borderRadius:12,cursor:"pointer",position:"relative",overflow:"hidden",gridRow:mobileHeight?undefined:(big?"1/3":undefined),background:bg||"linear-gradient(135deg,#1a2332 0%,#2d3f52 100%)",...heightStyle}}>
        <img src={img} alt={title} loading="lazy" width="400" height="300" style={{position:"absolute",right:0,bottom:0,height:"90%",maxWidth:"65%",objectFit:"contain",objectPosition:"right bottom",opacity:0.92}}/>
        ...
        <div style={{position:"relative",zIndex:1,padding:pad,display:"flex",flexDirection:"column",justifyContent:"flex-end",height:"100%"}}>
          ...
          <button style={{marginTop:small?8:12,padding:small?"6px 14px":"9px 22px",borderRadius:20,border:"none",background:"#E8700A",color:"#fff",fontSize:small?12:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",width:"fit-content"}}>{buttonLabel || thome("categories.explore")}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="hl" onClick={onClick} style={{borderRadius:12,cursor:"pointer",position:"relative",overflow:"hidden",gridRow:mobileHeight?undefined:(big?"1/3":undefined),...heightStyle}}>
      <img src={img} alt={title} loading="lazy" width="600" height="400" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}}/>
      ...
      <div style={{position:"relative",zIndex:1,padding:pad,display:"flex",flexDirection:"column",justifyContent:"flex-end",height:"100%"}}>
        ...
        <button style={{marginTop:small?8:12,padding:small?"6px 14px":"9px 22px",borderRadius:20,border:"none",background:"#4CAF50",color:"#fff",fontSize:small?12:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",width:"fit-content"}}>{buttonLabel || thome("categories.explore")}</button>
      </div>
    </div>
  );
}
