...
export default function PriceGate({ price, isLoggedIn, onLogin }) {
  ...
  if (isLoggedIn) return <span style={{fontSize:16,fontWeight:700,color:"#222"}}>{formatMoney(price, i18n.language)} <span style={{fontSize:11,fontWeight:400,color:"#888"}}>/{tcommon("pcs")}</span></span>;
  return (
