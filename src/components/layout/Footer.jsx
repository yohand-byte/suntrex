  ...
  return (
    <footer style={{padding:"40px 40px 24px",borderTop:"1px solid #e4e5ec"}}>
      <div style={{display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr 1fr",gap:40,marginBottom:32}}>
        ...
        <div><h4 style={{fontSize:13,fontWeight:600,marginBottom:12}}>Contact</h4>{["Aide","Vendre","À propos","Blog"].map(l=><div key={l} style={{fontSize:13,color:"#7b7b7b",marginBottom:8,cursor:"pointer"}}>{l}</div>)}</div>
        <div><h4 style={{fontSize:13,fontWeight:600,marginBottom:12}}>Catégories</h4>{["Panneaux","Onduleurs","Stockage","Montage"].map(l=><div key={l} style={{fontSize:13,color:"#7b7b7b",marginBottom:8,cursor:"pointer"}}>{l}</div>)}</div>
