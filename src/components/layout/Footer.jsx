  ...
  return (
    <footer style={{padding:isMobile?"32px 16px 20px":"40px 40px 24px",borderTop:"1px solid #e4e5ec"}}>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"1.5fr 1fr 1fr 1fr",gap:isMobile?20:40,marginBottom:isMobile?20:32}}>
        ...
        <div><h4 style={{fontSize:13,fontWeight:600,marginBottom:12}}>{t("footer.brands")}</h4><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 16px"}}>{["Jinko","LONGi","Trina","Huawei","BYD","Deye","SMA","Enphase"].map(l=><FooterLink key={l} label={l} onClick={()=>navigate(`/catalog/all?search=${l}`)} />)}</div></div>
      </div>
