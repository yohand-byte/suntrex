...
export default function AM({type}) {
  ...
  if (type === "catalog") return (<div style={B}>
    ...
    {s>=2&&<div style={{fontSize:11,color:"#4CAF50",marginBottom:12,fontWeight:500}}>{`47 ${t("mockups.catalog.offersFromSellers", { sellerCount: 12 })}`}</div>}
    {[{v:"SolarPro GmbH",f:"DE",r:"4.9",p:"1 180",d:3},{v:"PV Direct France",f:"FR",r:"4.8",p:"1 210",d:5},{v:"GreenEnergy BV",f:"NL",r:"4.7",p:"1 249",d:7}].map((r,i)=>s>=r.d&&(
      <div key={i} className="ar" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid #f0f0f0"}}>
        ...
        <div style={{textAlign:"right"}}><div style={{fontSize:15,fontWeight:700,color:"#E8700A"}}>{"€"+r.p}<span style={{fontSize:11,fontWeight:400}}>/pcs</span></div><div style={{fontSize:11,color:"#4CAF50"}}>{t("mockups.catalog.inStock")}</div></div>
      </div>
    ))}
    {s>=9&&<button style={{marginTop:12,width:"100%",padding:10,borderRadius:8,border:"none",background:"#E8700A",color:"#fff",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{t("mockups.catalog.viewAllOffers")}</button>}
  </div>);
  ...
  if (type === "chat") {
    ...
    return(<div style={B}>
      <div style={{fontSize:11,color:"#999",marginBottom:8,fontWeight:600}}>{t("mockups.chat.title")}</div>
      <div style={{background:"#fff",borderRadius:10,border:"1px solid #eee",padding:14,minHeight:220}}>
        ...
        {s>=11&&<div style={{textAlign:"center",padding:8,fontSize:11,color:"#4CAF50",fontWeight:500}}>{t("mockups.chat.agreementFound")}</div>}
      </div>
    </div>);
  }

  if (type === "payment") return(<div style={B}>
    <div style={{fontSize:11,color:"#999",marginBottom:16,fontWeight:600}}>{t("mockups.payment.title")}</div>
    <div style={{background:"#fff",border:"1px solid #eee",borderRadius:10,padding:16}}>
      {s>=1&&<div className="ar">{[["200x SUN2000-10KTL","€220 000"],[t("mockups.payment.deliverySuntrex"),"€1 200"],[t("mockups.payment.commission"),"€4 400"]].map(([l,v],i)=><div key={i} style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:13}}><span style={{color:"#7b7b7b"}}>{l}</span><span style={{fontWeight:500}}>{v}</span></div>)}</div>}
      {s>=3&&<div className="ar" style={{display:"flex",justifyContent:"space-between",borderTop:"2px solid #e4e5ec",paddingTop:12,fontSize:16}}><span style={{fontWeight:700}}>{t("mockups.payment.total")}</span><span style={{fontWeight:700,color:"#E8700A"}}>€225 600</span></div>}
      {s>=5&&<div className="ar" style={{background:"#f0fdf4",padding:10,borderRadius:8,fontSize:12,color:"#2e7d32",textAlign:"center",marginTop:14,fontWeight:500}}>{t("mockups.payment.escrowActivated")}</div>}
      {s>=7&&<div className="ar" style={{marginTop:12,display:"flex",gap:8}}>{["3D Secure","SCA Europe","Escrow"].map((label,i)=><div key={i} style={{flex:1,padding:8,textAlign:"center",background:"#f5f5f5",borderRadius:8,fontSize:11,fontWeight:600,color:"#4CAF50"}}>{label}</div>)}</div>}
      {s>=9&&<div style={{marginTop:12,height:44,borderRadius:8,background:"#E8700A",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14,fontWeight:600}}>{t("mockups.payment.paymentConfirmed")}</div>}
    </div>
  </div>);

  if (type === "delivery") {
    const st=[{s:t("mockups.delivery.orderConfirmed"),d:1},{s:t("mockups.delivery.shippedBySeller"),d:3},{s:t("mockups.delivery.pickedUpSuntrex"),d:5},{s:t("mockups.delivery.qualityControl"),d:7},{s:t("mockups.delivery.inTransit"),d:9},{s:t("mockups.delivery.deliveredPhotoConfirmed"),d:11}];
    return(<div style={B}>
      <div style={{fontSize:11,color:"#999",marginBottom:16,fontWeight:600}}>{t("mockups.delivery.title")}</div>
      <div style={{background:"#fff",border:"1px solid #eee",borderRadius:10,padding:16}}>
  ...
  if (type === "europe") return(<div style={B}>
    <div style={{fontSize:11,color:"#999",marginBottom:8,fontWeight:600}}>{t("mockups.europe.title")}</div>
  ...
  if (type === "createoffer") return(<div style={B}>
    <div style={{fontSize:11,color:"#999",marginBottom:12,fontWeight:600}}>{t("mockups.createOffer.title")}</div>
    <div style={{background:"#fff",border:"1px solid #eee",borderRadius:10,padding:16}}>
      {s>=1&&<div className="ar" style={{marginBottom:10}}><div style={{fontSize:11,color:"#999",marginBottom:4}}>{t("mockups.createOffer.product")}</div><div style={{padding:"8px 12px",background:"#f5f5f5",borderRadius:8,fontSize:13,fontWeight:500}}>Huawei SUN2000-10KTL-M2</div></div>}
      {s>=3&&<div className="ar" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        <div><div style={{fontSize:11,color:"#999",marginBottom:4}}>{t("mockups.createOffer.unitPrice")}</div><div style={{padding:"8px 12px",background:"#f5f5f5",borderRadius:8,fontSize:13,fontWeight:700,color:"#E8700A"}}>€1 180</div></div>
        <div><div style={{fontSize:11,color:"#999",marginBottom:4}}>{t("mockups.createOffer.stock")}</div><div style={{padding:"8px 12px",background:"#f5f5f5",borderRadius:8,fontSize:13}}>500 pcs</div></div>
      </div>}
      {s>=5&&<div className="ar" style={{marginBottom:10}}><div style={{padding:"8px 12px",background:"#f5f5f5",borderRadius:8,fontSize:13}}>{`SUNTREX Delivery — 3-5 ${t("mockups.createOffer.businessDays")}`}</div></div>}
      {s>=7&&<div className="ar" style={{padding:"8px 12px",background:"#f0fdf4",borderRadius:8,fontSize:12,color:"#2e7d32"}}>{t("mockups.createOffer.aiSuggestion")}</div>}
      {s>=9&&<button style={{marginTop:12,width:"100%",padding:10,borderRadius:8,border:"none",background:"#4CAF50",color:"#fff",fontSize:13,fontWeight:600,fontFamily:"inherit"}}>{t("mockups.createOffer.offerPublished")}</button>}
    </div>
  </div>);
  ...
  return(<div style={B}>
    ...
    {s>=6&&<div className="ar" style={{background:"#fff",border:"1px solid #eee",borderRadius:10,padding:12}}>
      ...
      {[["#ST-2847","€12 400",t("mockups.dashboard.inTransit")],["#ST-2846","€8 900",t("mockups.dashboard.delivered")],["#ST-2845","€22 100",t("mockups.dashboard.paid")]].map(([id,a,st],i)=>s>=8+i*2&&<div key={i} className="ar" style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:12,borderBottom:"1px solid #f5f5f5"}}><span style={{fontWeight:500}}>{id}</span><span>{a}</span><span style={{color:"#7b7b7b"}}>{st}</span></div>)}
    </div>}
  </div>);
}
